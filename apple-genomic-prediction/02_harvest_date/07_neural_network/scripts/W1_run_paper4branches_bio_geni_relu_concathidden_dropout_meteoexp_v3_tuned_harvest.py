...
################################################################################
### W1_run_paper4branches_bio_geni_relu_concathidden_dropout_meteoexp_v3_tuned_harvest.py
################################################################################
...
TRAIT = "Harvest_date"
MODEL_NAME = "paper4branches_bio_geni_relu_concathidden_dropout_meteoexp_v3_tuned"
...
# these 5 tuning splits are the same style you used before
TUNING_SPLITS = ["CV1_Split1", "CV2_Split2", "CV3_Split3", "CV4_Split4", "CV5_Split5"]
...
def load_shared_arrays():
    ...
    soil_all = np.load(NPY_DIR / "soil.npy").astype("float32")
    return weather_exp_all, pca_all, soil_all
...
def build_model(
    ...
    soil_input = tf.keras.layers.Input(shape=(n_soil,), name="Soil_Input")
    ...
    xw = tf.keras.layers.Dense(64, activation="relu", kernel_regularizer=reg)(weather_exp_input)
    xw = tf.keras.layers.Dense(32, activation="relu", kernel_regularizer=reg)(xw)
    xw = tf.keras.layers.Dense(16, activation="relu", kernel_regularizer=reg)(xw)
    xw = tf.keras.layers.Dense(8, activation="relu", kernel_regularizer=reg)(xw)

    xp = tf.keras.layers.Dense(128, activation="relu", kernel_regularizer=reg)(pca_input)
    xp = tf.keras.layers.Dense(128, activation="relu", kernel_regularizer=reg)(xp)
    xp = tf.keras.layers.Dense(64, activation="relu", kernel_regularizer=reg)(xp)
    xp = tf.keras.layers.Dense(32, activation="relu", kernel_regularizer=reg)(xp)
    xp = tf.keras.layers.Dense(16, activation="relu", kernel_regularizer=reg)(xp)
    xp = tf.keras.layers.Dense(8, activation="relu", kernel_regularizer=reg)(xp)

    xs = tf.keras.layers.Dense(64, activation="relu", kernel_regularizer=reg)(soil_input)
    xs = tf.keras.layers.Dense(64, activation="relu", kernel_regularizer=reg)(xs)
    xs = tf.keras.layers.Dense(32, activation="relu", kernel_regularizer=reg)(xs)
    xs = tf.keras.layers.Dense(16, activation="relu", kernel_regularizer=reg)(xs)
    xs = tf.keras.layers.Dense(8, activation="relu", kernel_regularizer=reg)(xs)
    ...
    bio_concat = tf.keras.layers.Concatenate(name="bio_concat")([gene_embedding, unmapped_embedding])
    ...
    all_concat = tf.keras.layers.Concatenate(name="global_concat")([xw, xp, xs, bio_embedding])
    ...
    model = tf.keras.Model(
        inputs=[weather_exp_input, pca_input, soil_input, mapped_input, unmapped_input],
        outputs=output,
        name=MODEL_NAME
    )
...
def run_one_split(split_name: str, params: dict, save_outputs: bool = False):
    ...
    weather_exp_all, pca_all, soil_all = load_shared_arrays()
    ...
    soil_subtrain = soil_all[subtrain_idx]
    soil_val = soil_all[val_idx]
    soil_test = soil_all[test_idx]
    ...
    soil_sub_sc = soil_scaler.fit_transform(soil_subtrain)
    soil_val_sc = soil_scaler.transform(soil_val)
    soil_test_sc = soil_scaler.transform(soil_test)
    ...
    if SCALE_TARGET:
        ...
        y_scaler = None
        y_subtrain_sc, y_val_sc, y_test_sc = y_subtrain, y_val, y_test
    ...
    history = model.fit(
        [weather_exp_sub_sc, pca_sub_sc, soil_sub_sc, X_mapped_subtrain_sc, X_unmapped_subtrain_sc],
        y_subtrain_sc,
        validation_data=(
            [weather_exp_val_sc, pca_val_sc, soil_val_sc, X_mapped_val_sc, X_unmapped_val_sc],
            y_val_sc
        ),
        epochs=MAX_EPOCHS,
    ...
    y_pred_test_sc = model.predict(
        [weather_exp_test_sc, pca_test_sc, soil_test_sc, X_mapped_test_sc, X_unmapped_test_sc],
        verbose=0
    ).ravel()

    y_pred_test = y_scaler.inverse_transform(y_pred_test_sc.reshape(-1, 1)).ravel() if y_scaler is not None else y_pred_test_sc
    ...
    if save_outputs:
        ...
        hist_df.to_csv(OUT_DIR / "loss_history" / f"loss_history_{split_name}.csv", index=False)
        ...
        metrics_df.to_csv(OUT_DIR / "metrics" / f"metrics_{split_name}.csv", index=False)
...
def run_tuning():
    tuning_summary_file = OUT_DIR / "tuning" / "tuning_results_summary.csv"
    tuning_partial_file = OUT_DIR / "tuning" / "tuning_results_summary_partial.csv"
    best_params_file = OUT_DIR / "tuning" / "best_params_Harvest_date.json"

    configs = all_param_configs(GRID)
    print(f"Running MODE = tune")
    ...
    for i, params in enumerate(configs, start=1):
        ...
        if key in existing_done:
            print(f"\nSkipping already completed config {i}/{len(configs)}: {params}")
            continue

        print(f"\nTuning config {i}/{len(configs)}: {params}")
        ...
        for split_name in TUNING_SPLITS:
            try:
                res = run_one_split(split_name, params, save_outputs=False)
                rows.append({
                    "Split": split_name,
                    "RMSE": res["rmse"],
                    "MAE": res["mae"],
        ...
        pd.DataFrame(partial_rows).to_csv(tuning_partial_file, index=False)

        print(f"Mean best val_loss on available tuning splits: {summary_row['mean_best_val_loss']:.6f}")
...
def run_training():
    ...
    for split_name in split_cols:
        if split_name in completed_splits:
            print(f"Skipping already completed split: {split_name}")
            continue

        print(f"Training final model on split: {split_name}")
        res = run_one_split(split_name, params, save_outputs=True)
        ...
        df = prepare_split_dataframe(meta, cv, inner, split_name)
        test_mask = df["Testing"] == 1
        pred_df = df.loc[test_mask, ["Envir", "Genotype", TRAIT, "Testing", "Split"]].copy()
...
def main():
    ...
    if MODE == "tune":
        ...
        raise ValueError("MODE must be 'tune' or 'train'")
